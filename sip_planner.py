...
def format_indian_currency(num):
    ...
    if len(s) <= 3:
        ...
        # This logic inserts commas for the lakhs/crores part
        rest_with_commas = ""
        while len(rest) > 2:
    ...
    with metric_col1:
        st.markdown(f"""
            <div class="metric-card">
                ...
                <div class="metric-value">{format_indian_currency(final_wealth)}</div>
    ...
    with metric_col2:
        st.markdown(f"""
            <div class="metric-card">
                ...
                <div class="metric-value">{format_indian_currency(final_invested)}</div>
    ...
    with metric_col3:
        st.markdown(f"""
            <div class="metric-card">
                ...
                <div class="metric-value">{format_indian_currency(final_returns)}</div>
...
# Format numbers using Indian Currency System
for col in ['Monthly SIP', 'Total Invested', 'Total Wealth', 'Wealth Gain']:
    display_data[col] = display_data[col].apply(lambda x: format_indian_currency(x))
